...
| `vite`    | `^7.3.6`  | [GHSA-fx2h-pf6j-xcff](https://github.com/advisories/GHSA-fx2h-pf6j-xcff) — `server.fs.deny` bypass on Windows alternate paths; also pulls a fixed `launch-editor` ([GHSA-v6wh-96g9-6wx3](https://github.com/advisories/GHSA-v6wh-96g9-6wx3)) | high     | `@sveltejs/kit`, `@sveltejs/vite-plugin-svelte`, `@tailwindcss/vite`, `vitest › @vitest/mocker` (a nested `vite@7.3.3` survives the direct devDependency bump) | every parent above resolves `vite >=7.3.5` on its own — check with `bun pm ls --all \| grep vite@`, which should show exactly one `vite` |
